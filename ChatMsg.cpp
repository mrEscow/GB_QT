...
ChatMsg::ChatMsg(QWidget *parent) : QWidget{parent}
{
    ...
    // te_font.setLetterSpacing (QFont :: PercentageSpacing, 100); // 300%, по умолчанию 100 // Установить интервал между словами%
    // te_font.setLetterSpacing (QFont :: AbsoluteSpacing, 0); // Установить интервал между словами 3 пикселя // Установить значение пикселя между интервалами
...
void ChatMsg::setText(QString text, QString time, QSize allSize, ChatMsg::User_Type userType)
{
    m_msg = text;
    m_userType = userType;
    m_time = time;
...
QSize ChatMsg::fontRect(QString str)
{
    m_msg = str;
    int minHei = 30;
    int iconWH = 40;
    ...
    QSize size = getRealString (m_msg); // весь размер
...
QSize ChatMsg::getRealString(QString src)
{
...
void ChatMsg::paintEvent(QPaintEvent *event)
{
    ...
    painter.setRenderHints (QPainter :: Antialiasing | QPainter :: SmoothPixmapTransform); // Сглаживание
    painter.setPen(Qt::NoPen);
    painter.setBrush(QBrush(Qt::gray));

    if (m_userType == User_Type::User_She) {// пользователь
        // аватар
    ...
    } else if (m_userType == User_Type::User_Me) {// я
        // аватар
    ...
    } else if (m_userType == User_Type::User_Time) {// время
        QPen penText;
